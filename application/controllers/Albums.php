...
class Albums extends CI_Controller {
	...
	public function detail() {
		...
		if ($query->num_rows() <= 0) {
			...
			if (is_numeric($dari)) {
				// Menambahkan logika untuk mengambil data foto dalam album
				$data['detailalbum'] = $this->model_utama->view_where_ordering_limit('gallery', array('id_album' => $row['id_album']), 'id_gallery', 'DESC', $dari, $config['per_page']);
			} else {
				redirect('main');
			}
